...
def test_signup(client, db):
    ...
    assert response.status_code == 200
    assert "アカウントが作成されました。ログインしてください。".encode('utf-8') in response.data
...
def test_add_task_api(logged_in_user, db):
    ...
    assert json_response['message'] == f"タスク「{task_data['title']}」を追加しました。"
...
def test_complete_task_api(logged_in_user, db):
    ...
    response = client.get(f'/complete/{task.id}', headers={'X-Requested-With': 'XMLHttpRequest'})
    assert response.status_code == 200
    json_response = response.get_json()
    assert json_response['status'] == 'success'
...
def test_delete_task_api(logged_in_user, db):
    ...
    response = client.get(f'/delete/{task.id}', headers={'X-Requested-With': 'XMLHttpRequest'})
    assert response.status_code == 200
    json_response = response.get_json()
    assert json_response['status'] == 'success'
    assert app_db.session.get(Task, task.id) is None
...
def test_cannot_operate_other_user_task(client, db):
    ...
    # 完了 (API)
    response_complete = client.get(f'/complete/{task_user1.id}', headers={'X-Requested-With': 'XMLHttpRequest'})
    assert response_complete.status_code == 404 # or 403 depending on implementation (first_or_404 vs explicit check)
    assert app_db.session.get(Task, task_user1.id).completed is False # 状態が変わっていないこと

    # 削除 (API)
    response_delete = client.get(f'/delete/{task_user1.id}', headers={'X-Requested-With': 'XMLHttpRequest'})
    assert response_delete.status_code == 404 # or 403
    assert app_db.session.get(Task, task_user1.id) is not None # 削除されていないこと

    # 編集ページGET (フォーム)
    response_edit_get = client.get(f'/edit/{task_user1.id}', follow_redirects=True)
